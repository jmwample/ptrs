...
pub(crate) struct O5NtorHandshake;

impl ClientHandshake for O5NtorHandshake {
...
impl std::str::FromStr for O5NtorPublicKey {
    type Err = Error;
    fn from_str(s: &str) -> std::prelude::v1::Result<Self, Self::Err> {
        ...
        Ok(O5NtorPublicKey::new(pk, id))
    }
}

#[allow(clippy::to_string_trait_impl)]
impl std::string::ToString for O5NtorPublicKey {
...
impl O5NtorSecretKey {
    ...
    /// Construct a new ['O5NtorSecretKey'] from a CSPRNG.
...
/// O5 helper trait to ensure that a returned key generator can be used
/// to create a usable codec and retrieve a session id.
pub trait O5Keygen: KeyGenerator + Into<O5Codec> {
    fn session_id(&mut self) -> [u8; SESSION_ID_LEN];
}

impl O5Keygen for NtorHkdfKeyGenerator {
    fn session_id(&mut self) -> [u8; SESSION_ID_LEN] {
        self.session_id
    }
}
